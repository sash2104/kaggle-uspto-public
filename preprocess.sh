...
## It took about 10 hours in my execution environment.
python script/1_convert_to_id.py -f SETTINGS.local.json -m find_twoshot
datadir=$(cat SETTINGS.local.json | jq -r .DATADIR)
find_twoshot_indir=$(cat SETTINGS.local.json | jq -r .FIND_TWOSHOT_INDIR)
...
python script/1_convert_to_id.py -f SETTINGS.local.json -m solver